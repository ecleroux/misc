...
#Specify file path
$filepath = "C:\counterlogs\processor-counter-logs-$currentdate.csv"

#Timestamp
$timestamp = Get-Date -Format "yyy-MM-dd HH:mm:ss"

#Log Processor Counters
$counters = (Get-Counter -ListSet Processor).Paths
...
Get-Counter $counters | Select-Object -expandProperty CounterSamples | group InstanceName | foreach {
    ...
    $ht.Add("Processor", $_.Name.ToUpper().Replace("_", ""))
